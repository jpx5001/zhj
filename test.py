...
def config_pull(context):
    ...
    config = context.get("hecate_sr_risklist_config")
    jsonobj = {}
    try:
...
class m_sr_risklist(object):
    ...
    def mapper(self, context):
        ...
        try:
            ...
            #
            # if risklistjson:
            #     risklistjson = risklistjson.get("product") # product
            #     if risklistjson:
            #         output["id_evidence_level"] = (
            #             str(risklistjson["id_evidence_level"])
            #             if "id_evidence_level" in risklistjson
            #                and risklistjson["id_evidence_level"]
            #             else "-999"
            #         )
            #
            #         output["id_forecast_level"] = (
            #             str(risklistjson["id_forecast_level"])
            #             if "id_forecast_level" in risklistjson
            #                and risklistjson["id_forecast_level"]
            #             else "-999"
            #         )
            #
            #         output["id_all_level"] = (
            #             str(risklistjson["id_all_level"])
            #             if "id_all_level" in risklistjson
            #                and risklistjson["id_all_level"]
            #             else "-999"
            #         )
            #
            #         output["idTmplevel"] = (
            #             str(risklistjson["idTmplevel"])
            #             if "idTmplevel" in risklistjson
            #                and risklistjson["idTmplevel"]
            #             else "-999"
            #         )
            #
            #         output["cell_evcellence_level"] = (
            #             str(risklistjson["cell_evcellence_level"])
            #             if "cell_evcellence_level" in risklistjson
            #                and risklistjson["cell_evcellence_level"]
            #             else "-999"
            #         )
            #
            #         output["cell_forecast_level"] = (
            #             str(risklistjson["cell_forecast_level"])
            #             if "cell_forecast_level" in risklistjson
            #                and risklistjson["cell_forecast_level"]
            #             else "-999"
            #         )
            #
            #         output["cell_all_level"] = (
            #             str(risklistjson["cell_all_level"])
            #             if "cell_all_level" in risklistjson
            #                and risklistjson["cell_all_level"]
            #             else "-999"
            #         )
            #
            #         output["cellTmplevel"] = (
            #             str(risklistjson["cellTmplevel"])
            #             if "cellTmplevel" in risklistjson
            #                and risklistjson["cellTmplevel"]
            #             else "-999"
            #         )

        except:
            log = context.getLog()
            log.error("Error <m_sr_risklist.py>获取外部风险名单失败 | %s" % (traceback.format_exc()))
        finally:
